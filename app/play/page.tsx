...
const ROOM_NAMES: Record<string, string> = {
  ...
  "2": "Beliefs & Rituals",
  "3": "Dvaravati Culture",
  "4": "Early Khmer Culture",
  "5": "Khmer Flourishing",
  "6": "Prosperous Phimai",
  "7": "Religion in Phimai",
  "8": "Phimai 12th–18th C.",
  "9": "Center of Greatness",
};
...
export default function PlayPage() {
  ...
  const rooms = [...new Set(me.tasks.map((t) => t.roomNumber))].sort((a, b) =>
    a.localeCompare(b),
  );

  const statusBadge = (s?: Submission) =>
    !s ? null : s.status === "correct" ? (
      <span className="ok">✓ +฿{s.moneyAwarded}</span>
    ) : s.status === "incorrect" ? (
      <span className="bad">✗ locked</span>
    ) : s.status === "pending_ai" ? (
      <span className="pend">
        🤖 AI checking
  ...
  return (
    <main className="page">
      ...
      <div className="card" style={{ marginTop: 12 }}>
        <h2>
          Team standings <span className="hint">อันดับทีม</span>
        </h2>
        {[...teams]
      ...
      {rooms.map((room) => (
        <div className="card" key={room}>
          <h2>
            📍 Room {room} · {ROOM_NAMES[room] ?? ""}
          </h2>
          {me.tasks
            .filter((t) => t.roomNumber === room)
            .map((t) => {
              const sub = subByTask.get(t.id);
              const locked =
                sub &&
                sub.status !== "pending_ai" &&
                sub.status !== "needs_manual";
              return (
                <Link
                  key={t.id}
                  href={locked ? "#" : `/play/task/${t.id}`}
                  className="chip"
                  style={{
                    display: "flex",
                    justifyContent: "space-between",
